...
class _PdfGeneratorState extends State<PdfGenerator> {
  ...
  Future<void> generatePdf(String organizationEmail, String adminName, String adminEmail, String adminContact, String orgname) async {
    ...
    // Split the entries into pages (e.g., 20 rows per page)
    const int rowsPerPage = 13; // Adjust this number based on your requirements
    int totalPages = (entries.length / rowsPerPage).ceil();

    for (int pageIndex = 0; pageIndex < totalPages; pageIndex++) {
      // Get the slice of entries for the current page
      final pageEntries = entries.skip(pageIndex * rowsPerPage).take(rowsPerPage).toList();

      // Add a page to the PDF with organization and admin details and table content
      pdf.addPage(
        pw.MultiPage(
          ...
          build: (pw.Context context) {
            ...
            return [
              ...
              pw.Header(
                ...
                child: pw.Center(
                  child: pw.Text(
                    'Entry Report for $orgname (Page ${pageIndex + 1}/$totalPages)',
              ...
              // Table Section with fetched entries (for current page)
  ...
  Future<List<Map<String, dynamic>>> fetchEntries(String organizationEmail) async {
    try {
      ...
      final snapshot = await FirebaseFirestore.instance
          ...
          .orderBy('EntryNo', descending: true) // Order entries by DateTime
          .get();
