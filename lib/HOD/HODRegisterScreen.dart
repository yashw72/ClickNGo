import 'package:clickngo/HOD/input3.dart';
...
import '../Authentication/input.dart'; // Importing the animation package

class HODRegister extends StatefulWidget {
  const HODRegister({Key? key}) : super(key: key);

  @override
  _HODRegisterState createState() => _HODRegisterState();
}
  ...
  Future<void> _signUp() async {
    if (_formKey.currentState?.validate() ?? false) {
      ...
      try {
        ...
        setState(() {
          _successMessage =
          'A verification email has been sent to ${_emailController.text.trim()}. Please verify your email to continue.';
        });
...
class SuccessAnimationScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    Future.delayed(const Duration(seconds: 3), () {
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (context) => DataScreen3(email: email)),
      );
    });
